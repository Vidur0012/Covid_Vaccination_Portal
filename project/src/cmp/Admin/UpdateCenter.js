...
function UpdateCenter(props){
    ...
    async function getData() {
        var res = await fetch("http://localhost:9700/centers/getCenterById/"+props.match.params.id);
        res = await res.json();
        if (res.wentWrong) {
            alert(res.message);
        }
    ...
    async function add(){
        ...
        if ( center_name && center_number && total_slots_d1!=='' && total_slots_d2!=='' && available_slots_d1!=='' && available_slots_d2!=='' && date && state && district && vaccine && vaccinator_name && center_pass && from && to && pincode!=='' && address) {
                ...
                var res = await fetch("http://localhost:9700/centers/updateCenter", {
                    method: "POST",
                    headers: {
    ...
    return(
        <div className="AddCenter">
            ...
            <button type="submit" onClick={add}>Update Center</button>
           
        </div>
    )
}

export default withRouter(UpdateCenter);